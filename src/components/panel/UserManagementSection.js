// src/components/panel/UserManagementSection.js
...
function UserManagementSection() {
  ...
  const roleColors = {
    ...
    curator: { bg: 'rgba(237, 108, 2, 0.1)', color: '#ed6c02' },
    admin: { bg: 'rgba(211, 47, 47, 0.1)', color: '#d32f2f' }
  };

  const roleIcons = {
    user: PersonIcon,
    curator: VerifiedUserIcon,
  ...
  const handleRoleChangeConfirm = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(`Successfully updated ${user.username}'s role to ${newRole}`);
