// src/components/panel/CityListSection.js - UPDATED WITH EDIT MODAL
...
import CityEditModal from './CityEditModal'; // Import the new modal
...
function CityListSection() {
    ...
    // Filters - improved state management
    ...
    const fetchCities = useCallback(async (page = 1, newFilters = filters) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Extract unique countries for filter dropdown - only on first load
    ...
    const handleEditCity = (cityId, event) => {
        event.stopPropagation(); // Prevent row click if we add row click later
    ...
    const handleCityUpdated = (updatedCity) => {
        // Update the city in the current list
        ...
        // Show success message (optional - the modal already shows success)
        console.log('City updated successfully:', updatedCity);
    };

    // Handle row click for edit (alternative to edit button)
    ...
    return (
        <Box sx={{ p: { xs: 2, md: 3 }, height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Cities Table - Fixed flick issue */}
            <Box sx={{ 
                flex: 1, 
                overflow: 'hidden', 
                display: 'flex', 
